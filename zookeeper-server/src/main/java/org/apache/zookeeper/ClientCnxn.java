    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        synchronized (packet) {
            if (requestTimeout > 0) {
                ...
                // Wait for request completion infinitely  阻塞等待 直到发送完成
                /**
                 * 在这里我们还看到，ZooKeeper方法中所谓的同步方法其实就是在Packet被提交到SendThread之后，陷入一个while循环，等待处理完成后再跳出的过程
                 */
