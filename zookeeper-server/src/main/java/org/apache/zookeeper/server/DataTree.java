...
public class DataTree {
    ...
    private static final String quotaChildZookeeper = quotaZookeeper//"zookeeper/quota"
     ...
     */
    private DataNode root = new DataNode(new byte[0], -1L, new StatPersisted()); // root

    /**
     * create a /zookeeper filesystem that is the proc filesystem of zookeeper
     */
    private final DataNode procDataNode = new DataNode(new byte[0], -1L, new StatPersisted()); //// 创建"/zookeeper"

    /**
     * create a /zookeeper/quota node for maintaining quota properties for
     * zookeeper
     */
    private final DataNode quotaDataNode = new DataNode(new byte[0], -1L, new StatPersisted()); //创建 "/zookeeper/quota"
    ...
    public void createNode(final String path, byte data[], List<ACL> acl,
        ...
        synchronized (parent) {
            ...
            DataNode child = new DataNode(data, longval, stat); //生成child node
            ...
            if (ephemeralType == EphemeralType.CONTAINER) {
                ...
                ttls.add(path);
            } else if (ephemeralOwner != 0) { //如果是临时的
        ...
        if(lastPrefix != null) { //找到Trie中最近的带有配额要求的路径
    ...
    public void deleteNode(String path, long zxid)
        ...
        synchronized (parent) {
            ...
            if (ephemeralType == EphemeralType.CONTAINER) {
                containers.remove(path); //parent的DataNode进行相应操作
            } else if (ephemeralType == EphemeralType.TTL) {
                ttls.remove(path); //parent的DataNode进行相应操作
            } else if (eowner != 0) { //如果当前节点是临时节点
    ...
    public Stat setData(String path, byte data[], int version, long zxid,
        ...
        String lastPrefix = getMaxPrefixWithQuota(path); //找到Trie中最近的带有配额要求的路径
        if(lastPrefix != null) {
